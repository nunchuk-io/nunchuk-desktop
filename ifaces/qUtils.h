...
void SetPassPhrase(const QString &storage_path, const QString &account, nunchuk::Chain chain, const QString &old_passphrase, const QString &new_passphrase);

nunchuk::Wallet ParseWalletDescriptor(const QString &descs, QWarningMessage &msg);

nunchuk::AnalyzeQRResult AnalyzeQR(const QStringList &qrtags);

nunchuk::Wallet ParseKeystoneWallet(nunchuk::Chain chain, const QStringList qrtags, QWarningMessage &msg);
...
bool SplitRequestToken(const QString &token, QString &signature, QString &fingerprint);

QString GetPartialSignature(const nunchuk::SingleSigner &signer, const QString &signed_psbt);

QString GetHealthCheckMessage(const QString &body, QWarningMessage &msg);

QString GetHealthCheckDummyTx(const nunchuk::Wallet &wallet, const QString &body, QWarningMessage &msg);

nunchuk::Transaction DecodeDummyTx(const nunchuk::Wallet &wallet, const QString &psbt, QWarningMessage &msg);

nunchuk::Transaction DecodeTx(const nunchuk::Wallet &wallet, const QString &psbt, const nunchuk::Amount &sub_amount, const nunchuk::Amount &fee,
                              const nunchuk::Amount &fee_rate, bool subtract_fee_from_amount, QWarningMessage &msg);

QString CreateRequestToken(const QString &signature, const QString &fingerprint, QWarningMessage &msg);
...
QStringList ExportQRTransaction(const QString &psbt, QWarningMessage &msg);
...
QString GetPrimaryKeyAddressFromMasterXprv(const QString &master_xprv);

QString GetMasterFingerprintFromMasterXprv(const QString &master_xprv);

QString SignLoginMessageWithMasterXprv(const QString &master_xprv, const QString &message);

bool CheckElectrumServer(const std::string &server, int timeout = 1);

bool IsValidPolicy(const std::string &policy);

std::string PolicyToMiniscript(const std::string &policy, const std::map<std::string, nunchuk::SingleSigner> &signers, nunchuk::AddressType address_type);

bool IsValidMiniscriptTemplate(const std::string &miniscript_template, nunchuk::AddressType address_type);

bool IsValidTapscriptTemplate(const std::string &tapscript_template, std::string &error);

std::string MiniscriptTemplateToMiniscript(const std::string &miniscript_template, const std::map<std::string, nunchuk::SingleSigner> &signers);

std::string TapscriptTemplateToTapscript(const std::string &tapscript_template, const std::map<std::string, nunchuk::SingleSigner> &signers,
                                         std::vector<std::string> &keypath);

nunchuk::ScriptNode GetScriptNode(const std::string &script, std::vector<std::string> &keypaths);

std::vector<uint8_t> HashPreimage(const std::vector<uint8_t> &preimage, nunchuk::PreimageHashType hashType);

std::string RevealPreimage(const std::string &psbt, nunchuk::PreimageHashType hashType, const std::vector<uint8_t> &hash, const std::vector<uint8_t> &preimage,
                           QWarningMessage &msg);

bool IsPreimageRevealed(const std::string &psbt, const std::vector<uint8_t> &hash);

std::string ExpandingMultisigMiniscriptTemplate(int m, int n, int new_n, bool reuse_signers, const nunchuk::Timelock &timelock,
                                                nunchuk::AddressType address_type, QWarningMessage &msg);

std::string DecayingMultisigMiniscriptTemplate(int m, int n, int new_m, bool reuse_signers, const nunchuk::Timelock &timelock,
                                               nunchuk::AddressType address_type, QWarningMessage &msg);

std::string FlexibleMultisigMiniscriptTemplate(int m, int n, int new_m, int new_n, bool reuse_signers, const nunchuk::Timelock &timelock,
                                               nunchuk::AddressType address_type, QWarningMessage &msg);

std::string ZenHodlMiniscriptTemplate(int m, int n, const nunchuk::Timelock &timelock, nunchuk::AddressType address_type, QWarningMessage &msg);

std::vector<nunchuk::UnspentOutput> GetTimelockedCoins(const std::string &miniscript, const std::vector<nunchuk::UnspentOutput> &coins, int64_t &max_lock_value,
                                                       int chain_tip);

std::vector<nunchuk::CoinsGroup> GetCoinsGroupedBySubPolicies(const nunchuk::ScriptNode &script_node, const std::vector<nunchuk::UnspentOutput> &coins,
                                                              int chain_tip);
...
QString GenerateColdCardHealthCheckMessage(const QString &derivation_path, const QString &message, nunchuk::AddressType address_type, QWarningMessage &msg);

QStringList ExportBBQRJSON(const QString &value, QWarningMessage &msg);

QString ExtractColdcardMessageSignature(const QStringList &qr_data, QWarningMessage &msg);

QString ExtractColdcardMessageSignature(const QString &value, QWarningMessage &msg);
} // namespace qUtils
#endif // QUTILS_H
