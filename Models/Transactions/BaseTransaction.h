...
class BaseTransaction : public QStateFlow {
    Q_OBJECT
    ...
    Q_PROPERTY(int                  pendingSignatures       READ pendingSignatures       NOTIFY nunchukTransactionChanged)
    ...
    Q_PROPERTY(int                  addressType             READ addressType             CONSTANT)
    Q_PROPERTY(int                  walletType              READ walletType              CONSTANT)
    Q_PROPERTY(bool                 useScriptPath           READ useScriptPath           CONSTANT)
    Q_PROPERTY(bool                 isClaimTx               READ isClaimTx               CONSTANT)
    Q_PROPERTY(bool                 isDummyTx               READ isDummyTx               CONSTANT)
    Q_PROPERTY(QString              groupTransactionState   READ groupTransactionState   NOTIFY nunchukTransactionChanged)

  public:
    BaseTransaction();
    BaseTransaction(const nunchuk::Transaction &tx);
    ~BaseTransaction();
    ...
    // For taproot
    SingleSignerListModel *taprootKeysetSigners(const QWalletPtr &wallet);
    SingleSignerListModel *taprootMiniscriptSigners(const QWalletPtr &wallet);
    ...
    void setPendingSignatures(int pending);
