...
BaseWallet::BaseWallet(const nunchuk::Wallet &w)
    : m_signers(QSingleSignerListModelPtr(new SingleSignerListModel())), m_transactionHistory(QTransactionListModelPtr(new TransactionListModel())),
...
int BaseWallet::walletAddressType() {
    if (m_walletAddressType == (int)nunchuk::AddressType::ANY) {
        ...
        std::set<int> valid_numbers = {(int)nunchuk::AddressType::LEGACY, (int)nunchuk::AddressType::NESTED_SEGWIT, (int)nunchuk::AddressType::NATIVE_SEGWIT,
...
void BaseWallet::updateSignMessage(const QString &xfp, int wallet_type, const QString &message, const QString &derivationPath) {
    ...
    runInConcurrent(
        [safeThis, wallet_type, xfp, derivationPath, message]() -> DataStruct {
