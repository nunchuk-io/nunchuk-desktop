...
QGroupWalletHealthCheck::QGroupWalletHealthCheck(WalletId wallet_id) : QBasePremium(wallet_id) {
        ...
        [safeThis, dashboard](QJsonArray statuses) {
            ...
            for (const auto &obj : statuses) {
                ...
                if (last.isNull()) {
                    ...
                    } else if (time_distance > safeThis->SixMonthToMillis() && time_distance < safeThis->YearToMillis()) {
...
bool QGroupWalletHealthCheck::HealthCheckPendingForTx(const QString &dummy_transaction_id, std::function<void(bool)> callback) {
    if (auto dashboard = dashBoardPtr()) {
        QJsonObject output;
        QString errormsg = "";
        bool ret{false};
...
void QGroupWalletHealthCheck::AddOrUpdateKeyHealthReminder(const QStringList xfps, const QString frequency, const QString start_date_millis) {
    if (auto dashboard = dashBoardPtr()) {
        QPointer<QGroupWalletHealthCheck> safeThis(this);
        runInConcurrent(
            [safeThis, xfps, frequency, start_date_millis]() -> bool {
