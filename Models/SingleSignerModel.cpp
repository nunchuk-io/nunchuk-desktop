...
QSingleSigner::QSingleSigner(const nunchuk::SingleSigner &singleKey) : isDraft(false) {
    convert(singleKey);
    QQmlEngine::setObjectOwnership(this, QQmlEngine::CppOwnership);

    connect(
        this, &QSingleSigner::requestResolveSignerName, this,
        [this](const QString &xfp) {
            QString resolved = "";
            ...
            if (!resolved.isEmpty()) {
                DBG_INFO << "Resolved m_signername"
                         << "xfp" << xfp << "index" << accountIndex() << "from" << m_signername << "to" << resolved << this;
...
nunchuk::SignerTag QSingleSigner::signerTag() const {
    ...
    tags.erase(std::remove_if(tags.begin(), tags.end(), [](const nunchuk::SignerTag &tag) { return tag == nunchuk::SignerTag::INHERITANCE; }), tags.end());
    if (tags.empty()) {
...
bool QSingleSigner::isMine() const {
    ...
    auto it = std::find_if(signers.begin(), signers.end(),
                           [this](const nunchuk::SingleSigner &signer) { return signer.get_master_fingerprint() == masterFingerPrint().toStdString(); });
...
bool SingleSignerListModel::removeSingleSignerByType(const int type) {
    beginResetModel();
    m_data.erase(std::remove_if(m_data.begin(), m_data.end(), [type](const auto &item) { return item.data()->signerType() == type; }), m_data.end());
...
QSingleSignerPtr SingleSignerListModel::getSingleSignerBy(const QString &xfp, int account_index) {
    foreach (QSingleSignerPtr i, m_data) {
        if (qUtils::strCompare(xfp, i.data()->masterFingerPrint()) && account_index == i.data()->accountIndex()) {
            return i;
        }
    }
    return NULL;
...
bool SingleSignerListModel::containsSigner(const QString &xfp, const QString &path) {
    foreach (QSingleSignerPtr i, m_data) {
        if (path.isEmpty() && i.data()->derivationPath().isEmpty() && qUtils::strCompare(xfp, i.data()->masterFingerPrint())) {
            return true;
        } else if (qUtils::strCompare(path, i.data()->derivationPath()) && qUtils::strCompare(xfp, i.data()->masterFingerPrint())) {
            return true;
        }
    }
    return false;
...
QSharedPointer<SingleSignerListModel> SingleSignerListModel::cloneKeysets(std::vector<nunchuk::KeysetStatus> keyset_status) const {
    ...
    for (int i = 0; i < keyset_status.size(); i++) {
        ...
        for (std::map<std::string, bool>::iterator it = keystatus.begin(); it != keystatus.end(); it++) {
            ...
            // FIXME - TRICK -->>
            std::set<int> valid_numbers = {(int)nunchuk::TransactionStatus::CONFIRMED, (int)nunchuk::TransactionStatus::READY_TO_BROADCAST,
