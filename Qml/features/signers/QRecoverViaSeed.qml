    ...
    content: Item {
        ...
        Item {
            ...
            function searchingText(str){
                modelSearch.clear()
                for(var i = 0; i < vm.suggestMnemonics.length; i++){
                    if(str === ""){
                        var data = {'mnemonic': vm.suggestMnemonics[i]};
                        modelSearch.append(data)
                    }
                    else{
                        var currentText = vm.suggestMnemonics[i]
                        if (currentText.toLowerCase().includes(str.toLowerCase())){
                            var datafilter = {'mnemonic': vm.suggestMnemonics[i]};
