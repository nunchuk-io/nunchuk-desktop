...
QtObject {
    ...
    // No HOME list owns focus until the loaded models are reconciled.
    property int    listFocusing: -1
    property string recoverSoftwareType: "seed"
    property bool   showOthersKeyset: true

    // Session-scoped recents shared by all chat composers. Reassigning the
    // array is intentional so QML bindings are notified after each update.
    property var chatRecentEmojis: []

    function rememberChatEmoji(emoji) {
        var next = []
        for (var i = 0; i < chatRecentEmojis.length; ++i) {
            if (chatRecentEmojis[i] !== emoji) {
                next.push(chatRecentEmojis[i])
            }
        }
        next.unshift(emoji)
        chatRecentEmojis = next.slice(0, 24)
    }

    // Transient manual-fee state while QCreateTransaction is destroyed and
    // recreated during the coin-selection round trip.
