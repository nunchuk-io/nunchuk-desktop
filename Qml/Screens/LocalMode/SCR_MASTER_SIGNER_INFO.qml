...
QScreen {
    ...
    Loader {
        ...
        sourceComponent: {
            if (action_type === eKEY_INFO) {
                if (flow === AlertType.HEALTH_CHECK_FREQUENCY_REPEAT) {
                    return _checkFrequencyRepeat
                }
                else {
    ...
    Component {
        ...
        QHealthCheckFrequencyRepeat {
            onHealthCheckBack: {
                QMLHandle.sendEvent(EVT.EVT_MASTER_SIGNER_INFO_HEALTH_CHECK, obj)
            }
            onHealthCheckSave: {
                QMLHandle.sendEvent(EVT.EVT_MASTER_SIGNER_INFO_HEALTH_CHECK, obj)
            }
    ...
    Component {
        ...
        QKeyInfo {
            ...
            onHealthReminderClicked: {
                ...
                QMLHandle.sendEvent(EVT.EVT_MASTER_SIGNER_INFO_HEALTH_CHECK, check)
            }
