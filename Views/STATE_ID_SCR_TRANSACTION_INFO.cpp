...
void EVT_TRANSACTION_EXPORT_REQUEST_HANDLER(QVariant msg) {
    ...
    if (file_path != "") {
        ...
        bool ret = bridge::nunchukExportTransaction(AppModel::instance()->walletInfo()->walletId(), AppModel::instance()->transactionInfo()->txid(), file_path,
                                                    msgwarning);
...
void EVT_TRANSACTION_EXPORT_QRCODE_HANDLER(QVariant msg) {
    ...
    if (AppModel::instance()->walletInfo() && AppModel::instance()->transactionInfo()) {
        ...
        if (qUtils::strCompare(qrtype, "QR-transaction")) {
            QStringList qrtags =
                bridge::nunchukExportQRTransaction(AppModel::instance()->walletInfo()->walletId(), AppModel::instance()->transactionInfo()->txid(), msgwarning);
...
void EVT_TRANSACTION_IMPORT_QRCODE_HANDLER(QVariant msg) {
    if (AppModel::instance()->transactionInfo() && AppModel::instance()->walletInfo()) {
        AppModel::instance()->walletInfo()->SignAsisstedTxs(AppModel::instance()->transactionInfo()->txid(), AppModel::instance()->transactionInfo()->psbt(),
