...
void EVT_UTXOS_ITEM_SELECTED_HANDLER(QVariant msg) {
    if(msg.toInt() >= 0 && AppModel::instance()->walletInfo() && AppModel::instance()->walletInfo()->utxoList()){
        QUTXOPtr info = AppModel::instance()->walletInfo()->utxoList()->getUTXOByIndex(msg.toInt());
        if(info){
            AppModel::instance()->walletInfo()->setUtxoInfo(info);
        }
    }
}
